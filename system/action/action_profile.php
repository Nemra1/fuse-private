...
if(isset($_POST['save_shared'])){
     if(boomLogged()){
        ...
        $updare_query = cl_update_user_data($data['user_id'],$update_privace);         
    }
}
...
if(isset($_POST['update_pushId'])){
     if(boomLogged()){
    	...
    	$mysqli->query("UPDATE boom_users SET push_id = '$push_id' WHERE user_id = '{$data['user_id']}'");
    }
}
?>