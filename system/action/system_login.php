				...
				if ($validate->num_rows > 0) {
					...
					if (strlen($db_password) === 40 || strpos($db_password, '$2y$') !== 0) {
						if (encrypt($input_password) === $db_password) {
							...
							$stmt_update = $mysqli->prepare("UPDATE boom_users SET user_password = ? WHERE user_id = ?");
					...
					if (password_verify($input_password, $db_password)) {
						...
						$stmt_update_ip = $mysqli->prepare("UPDATE boom_users SET user_ip = ?, session_id = ?, join_msg = '0', user_roomid = '0' WHERE user_id = ?");
