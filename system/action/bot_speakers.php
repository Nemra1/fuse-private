...
if($f == "bot_speakers") {
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && $s === "del_bot") {
		...
		if ($bot_id === false) {
			...
			echo json_encode(["status" => 400, "error" => "Invalid bot ID."]);
			exit();
		}
		...
		if ($del_query) {
			...
			echo json_encode(["status" => 200, "message" => "Bot deleted successfully."]);
			exit();
		} else {
			// Log error for debugging (optional)
			error_log("Failed to delete bot with ID: $bot_id. DB error: " . $db->getLastError());
			http_response_code(500);
			echo json_encode(["status" => 500, "error" => "Failed to delete bot."]);
			exit();
		}
	}
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && $s === "admin_bot_byroom") {
		...
		if ($group_id === false) {
			http_response_code(400);
			echo json_encode(["status" => 400, "error" => "Invalid room ID."]);
			exit();
		}
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && $s === "admin_bot_info") {
		...
		if ($bot_id === false || $group_id === false) {
			http_response_code(400);
			echo json_encode(["status" => 400, "error" => "Invalid bot ID or group ID."]);
			exit();
		}
	...
	if($_SERVER['REQUEST_METHOD'] === 'POST' && $s === "update_bot") {
		...
		$fuse_bot_status = filter_input(INPUT_POST, 'fuse_bot_status', FILTER_SANITIZE_STRING);
		$fuse_bot_type = filter_input(INPUT_POST, 'fuse_bot_type', FILTER_SANITIZE_STRING);
		$fuse_bot_line = filter_input(INPUT_POST, 'fuse_bot_line', FILTER_SANITIZE_STRING);
		// Check for missing or invalid inputs
		if ($bot_id === false || $bot_user_id === false || empty($fuse_bot_status) || empty($fuse_bot_type) || empty($fuse_bot_line)) {
			http_response_code(400);
			echo json_encode(["status" => 400, "error" => "Invalid or missing input data."]);
			exit();
		}
		// Set update time
		$time = time();
	...
	if ($s === "add_bot") {
		...
		if (!$post_data || !$post_data["fuse_bot_id"] || !$post_data["group_id"]) {
			http_response_code(400);
			echo json_encode(["status" => 400, "message" => "Invalid bot or group ID."]);
			exit();
		}
		...
		if ($insert_result) {
			...
			echo json_encode(["status" => 201, "message" => "Bot has been added successfully"]);
		} else {
			http_response_code(500);
			echo json_encode(["status" => 500, "message" => "Database error: Unable to add bot"]);
		}
		exit();
	}
	if ($s === "update_bot_set") {
		...
		if (!$stmt) {
			http_response_code(500);
			echo json_encode(["status" => 500, "message" => "Database error: Failed to prepare statement."]);
			exit();
		}
		...
		if ($execute_success && $stmt->affected_rows > 0) {
			http_response_code(200);
			echo json_encode(["status" => 200, "message" => "Bot settings updated successfully."]);
		} else {
			http_response_code(500);
			echo json_encode(["status" => 500, "message" => "No changes made or update failed."]);
		}
	...
	if ($s === "allow_bot" && boomLogged()) {
		...
		if (!isset($_POST['allow_bot'])) {
			http_response_code(400);
			echo json_encode(["status" => 400, "message" => "Missing 'allow_bot' parameter."]);
			exit();
		}
		...
		if ($allow_bot === false) {
			http_response_code(400);
			echo json_encode(["status" => 400, "message" => "'allow_bot' must be either 0 or 1."]);
			exit();
		}
		...
		if ($execute_success && $stmt->affected_rows > 0) {
			http_response_code(200);
			echo json_encode(["status" => 200, "message" => "Bot setting updated successfully."]);
		} else {
			http_response_code(500);
			echo json_encode(["status" => 500, "message" => "Failed to update the bot setting."]);
		}
		// Close the statement
		$stmt->close();
		exit();
